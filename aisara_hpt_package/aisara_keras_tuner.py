...
class Hp:
    ...
    def numrange(self, name, min, max, type='linear'):
        if type.lower().strip()== 'linear':
            ...
            raise UnknownArgument('unknown type found!!... the type for numerical hyperparameters should be either "linear" or "log"')
    ...
    def remove_hp (self, name):
        if name.strip() in self.hps_dict_num.keys():
            ...
            raise UnknownArgument('hyperparameter is not defined')
